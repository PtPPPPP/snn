...
type Snapshot={time:number;selection:string;context:string};
type Turn={question:string;answer:string;label:string;context:string;complete:boolean};
export default function RocketTutor({snapshot,locked=false,onFollow,onLock}:{snapshot:Snapshot;locked?:boolean;onFollow?:()=>void;onLock?:()=>void}){
 ...
 async function ask(value:string){
  ...
  const label=`${frozen.time.toFixed(2)} 秒 · ${frozen.selection}`;
  ...
  const prompt='你是 SNN 火箭实验的高中生数学助教。用简明中文，先直接回答，再用当前数据举例，通常不超过250字。用 a1+a2+…+an+b 解释求和，不用矩阵和复杂符号。下方是提问时冻结的模型真实数据；飞行继续播放，不要把之后的状态当作该问题的状态。区分已知计算与推测：激活值和权重不能证明神经元有特定语义，不能声称相关性就是因果。没有训练记录，不能编造训练成绩。你只解释，不能控制火箭。缺少证据时说明缺少什么。\n';
 ...
 return <section className={styles.tutor} aria-label="实验 AI 答疑"><h2>问问 SNN AI</h2><div className={styles.questionContext}><b>{locked?'已记录':'跟随当前飞行'} · {snapshot.time.toFixed(2)} 秒 · {snapshot.selection}</b>{locked?<button onClick={onFollow}>跟随当前</button>:<button onClick={onLock}>锁定这一刻</button>}</div>
 <form onSubmit={e=>{e.preventDefault();void ask(question);}}><label htmlFor="rocket-question">你的问题</label><textarea id="rocket-question" rows={2} maxLength={1000} value={question} onChange={e=>setQuestion(e.target.value)} placeholder="例如：为什么速度是负数，油门却在变大？"/><div><small>将附上 {snapshot.time.toFixed(2)} 秒 · {snapshot.selection}</small>{busy?<button type="button" onClick={()=>controller.current?.abort()}>停止回答</button>:<button disabled={!question.trim()} type="submit">发送问题</button>}</div></form>
 <div className={styles.tutorQuestions}>{['解释当前结果','这个正负数是什么意思？'].map(q=><button key={q} disabled={busy} onClick={()=>void ask(q)}>{q}</button>)}</div>
