...
export class FileIngestionService {
  ...
  async ingest({ workspaceId, originalName, contentType = "application/octet-stream", body }) {
    ...
    const file = Object.freeze({ fileId, originalName: safeName, virtualPath: safeName, storedName, size: bytes.length, contentType: normalizeType(contentType), kind, sha256: createHash("sha256").update(bytes).digest("hex"), updatedAt: Date.now() });
    try {
  ...
  async writeEditableText({ workspaceId, virtualPath, content, expected }) {
    ...
    const storedName = `.snn-upload-${randomUUID()}`;
    const next = Object.freeze({ fileId, originalName: current?.originalName ?? basename(resolved.virtualPath), virtualPath: resolved.virtualPath, storedName, size: bytes.length, contentType: current?.contentType ?? "text/plain", kind: "text", sha256: createHash("sha256").update(bytes).digest("hex"), updatedAt: Date.now() });
...
export function isEditableTextFile(file) { return file.kind === "text" && isEditableTextPath(file.virtualPath ?? file.originalName); }
